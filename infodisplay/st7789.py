...
class ST7789:
    ...
    def __init__(
        ...
        self._dma = DmaManager(spi, self.width, spi_id=spi_id, bytes_per_pixel=2)
        ...
        if self.source_color_mode == 'RGB565':
            ...
            if self._dma.active:
                for row in range(rh):
                    ...
                    line_conv(buf, src, (y + row) * width + x, rw if scale == 1 else rw)
                    # For scale > 1, _upscale_line handles the horizontal repeat
                    # But we need to handle vertical repeat
                    for _ in range(scale):
                ...
                for row in range(rh):
                    ...
                    if scale == 1:
                        line_conv(lb, src, (y + row) * width + x, rw)
                    else:
                        line_conv(lb, src, (y + row) * width + x, rw, scale)
                    for _ in range(scale):
                        self._spi.write(lb)
        else: # RGB332
