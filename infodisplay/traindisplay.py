...
Departure = namedtuple('Departure', 'scheduled_time expected_time destination train_class cancelled delayed')
...
class TrainDisplay:
    ...
    def __draw_departure_row(self, departure, y_offset):
        ...
        textbox.draw_textbox(self.display, train_class, time_width + destination_width, y_offset, train_class_width, 20, color=row_pen, font='small')
        textbox.draw_textbox(self.display, expected, time_width + destination_width + train_class_width, y_offset, expected_width, 20, color=row_pen, font='small')
    ...
    async def fetch_departures(self):
        try:
            ...
            # Format: [std, station, class, atd, std, station, class, atd, ...]
            self.departures = []
            for i in range(0, len(raw_data), 4):
                if i + 3 < len(raw_data):
                    std = raw_data[i]
                    station = raw_data[i + 1]
                    train_class = raw_data[i + 2]
                    atd = raw_data[i + 3]
