...
class HassBme280:
    def __init__(self, hass, bus, scl, sda, temperature_config, pressure_config, humidity_config):
        self.i2c = machine.I2C(bus, scl=scl, sda=sda)
    ...
    def create(provider):
        ...
        return HassBme280(provider['hass.Hass'], config.get('bus', 0), config['scl'], config['sda'], config['temperature'], config['pressure'], config['humidity'])
