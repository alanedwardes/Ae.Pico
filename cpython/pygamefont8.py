...
class Font8:
    ...
    @classmethod
    def draw_text(cls, surface, text, x, y, color=(255,255,255), spacing=1, scale=1):
        ...
        for char in text:
            ...
            if 32 <= index <= 126:
                font_index = index - 32
                cx += (cls.widths[font_index] + spacing) * scale
            else:
                cx += (cls.max_width + spacing) * scale

    @classmethod
    def measure_text(cls, text, scale=1, spacing=1):
        """Measure the width in pixels of the given text string, with scaling."""
        width = 0
        for char in text:
            index = ord(char)
            if 32 <= index <= 126:
                font_index = index - 32
                width += (cls.widths[font_index] + spacing) * scale
            else:
                width += (cls.max_width + spacing) * scale
        if len(text) > 0:
            width -= spacing * scale  # Remove trailing spacing
        return width
