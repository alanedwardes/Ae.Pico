...
class IndexController:
    ...
    def serve(self, headers, connection):
        ...
        connection.write(b'<style>body{form{display:inline}</style>')        
        connection.write(b'<h1>Management Dashboard</h1>')
        connection.write('<p>{}</p>'.format(uos.uname()).encode('utf-8'))
        connection.write(b'<ul>')
        connection.write('<li>Uptime: {:.0f}s</li>'.format(uptime_ms / 1000).encode('utf-8'))
        connection.write('<li>CPU frequency: {:.0f} MHz</li>'.format(machine.freq() / 1_000_000).encode('utf-8'))
        connection.write('<li>CPU temperature: {:.0f} celcius</li>'.format(cpu_temp()).encode('utf-8'))
        connection.write('<li>Unique ID: {}</li>'.format(unique_id()).encode('utf-8'))
        connection.write('<li>Current time: {}</li>'.format(current_time()).encode('utf-8'))
        connection.write(b'</ul>')
        ...
        connection.write('<p>Free Memory: {:.2f} KB</p>'.format(free_memory / KB).encode('utf-8'))
        connection.write(b'<h2>Filesystem</h2>')
        connection.write('<p>Free Space: {:.2f} KB</p>'.format(free_space / KB).encode('utf-8'))
        ...
        def write_file(parent, file):
            ...
            connection.write(b'<td>' + path + '</td>')
            connection.write(b'<td>{:.2f} KB</td>'.format(file[3] / KB))
            connection.write(b'<td>')
            connection.write(b'<form action="delete" method="post"><input type="hidden" name="filename" value="' + path + '"/><button>Delete</button/></form>')
            connection.write(b'<form action="download" method="post"><input type="hidden" name="filename" value="' + path + '"/><button>Download</button/></form>')
            connection.write(b'</td>')
            connection.write(b'</tr>')
...
class DeleteController:
    ...
    def serve(self, headers, connection):
        ...
        try:
            uos.unlink(filename)
            connection.write(b'<p>Deleted "{}"</p>'.format(filename))
        except Exception as e:
            connection.write(b'<p>Error deleting "{}": {}</p>'.format(filename, e))
            
        connection.write(b'<p><a href="/">Back</a></p>')
...
class UploadController:    
    ...
    def serve(self, headers, connection):
        ...
        connection.write(b'<p>{} uploaded ({} bytes)'.format(filename, content_size))
        connection.write(b'<p><a href="/">Back</a></p>')
...
class DownloadController:
    ...
    def serve(self, headers, connection):        
        ...
        try:
            ...
            connection.write(b'<p>Error reading "{}": {}</p>'.format(filename, e))
            connection.write(b'<p><a href="/">Back</a></p>')
